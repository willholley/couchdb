...
class IndexSelectionTests(mango.UserDocsTests):
    ...
    # test range scan on first column in index (valid), second column exists (valid)

    # test range scan on first column in index (valid), second column does not exists (not valid)

    # test range scan on first column in index and second column in index (not valid)

    # test range scan on second column in index (not valid)

    # test range scan on second column in index, first column exists (not valid)

    # test range scan on second column in index, first column is equal (valid)

    # test range scan on second column in index, first column is equal, third column exists (valid)    
